...
public class Shader {
    ...
    public void addShader(String filePath, int type) {
        ...
        if (result == GL_FALSE) {
            ...
            log.error("Failed to compile " + (type == GL20.GL_VERTEX_SHADER ? "vertex" : "fragment") + " shader!");
    ...
    public void setUniMat4f(String name, Matrix4f matrix) {
        ...
        if(uniformLocation != -1) {
            ...
            log.debug("The name " + name + " does not correspond to an active uniform variable in the current shader.");
        }
    }
